...
def typewriter_text(text, speed=0.05):
    full_text = f'<div class="typewriter-text">{text}</div>'
    placeholder = st.empty()
    for i in range(len(text) + 1):
        placeholder.markdown(f'<div class="typewriter-text">{text[:i]}</div>', unsafe_allow_html=True)
        time.sleep(speed)
...
def main():
    ...
    if st.session_state.stage == 'intro':
        intro_text1 = typewriter_text("Welcome to Spotify Music Explorer! Here you can discover detailed information about artists and tracks.")
        st.session_state.typed_text.append(intro_text1)
        time.sleep(1)
        intro_text2 = typewriter_text("Let's start by searching for an artist or a track.")
        st.session_state.typed_text.append(intro_text2)
        time.sleep(1)
        ...
        for text in st.session_state.typed_text:
        ...
        search_text = typewriter_text("Searching the vast world of music... 🎵")
        st.session_state.typed_text.append(search_text)
        ...
        if results:
            result_text = typewriter_text("Great! Here's what I found for you:")
            st.session_state.typed_text.append(result_text)
            ...
            if st.session_state.search_type == "Artist":
                ...
                if artists:
                    ...
                    if artist_info:
                        ...
                        tracks_text = typewriter_text("Check out the artist's top tracks:")
                        st.session_state.typed_text.append(tracks_text)
                    ...
                    not_found_text = typewriter_text("Hmm, I couldn't find any artists with that name. Let's try a different search!")
                    st.session_state.typed_text.append(not_found_text)
            else:
                tracks = results['tracks']['items']
                tracks_text = typewriter_text("Here are some tracks that match your search:")
                st.session_state.typed_text.append(tracks_text)
                for track in tracks:
                    col1, col2 = st.columns([1, 2])
                    with col1:
            ...
            not_found_text = typewriter_text("Oops! I couldn't find anything matching your search. Let's try something else!")
            st.session_state.typed_text.append(not_found_text)
